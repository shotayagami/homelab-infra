...
### Phase B 部分実施 (2026-05-18 後半、memory hotplug 増)

KSM 強化 + worker1 swap 投入 ([docs/proxmox-zabbix-monitoring.md](proxmox-zabbix-monitoring.md) §RKE2 メモリ圧緩和) で PVE host available が回復したため、同日中に memory hotplug で cp1/worker1 を増。Phase 0 → Phase B で現状 **7c / 20 GiB**:

| VM | Phase 0 後 | Phase B 後 (現状) |
|---|---|---|
| cp1 (110) | 4c / 9 GiB | 4c / **11 GiB** |
