...
| 項目 | 値 |
|---|---|
| 本体スクリプト | `/usr/local/bin/network-watchdog.sh` |
| systemd unit | `network-watchdog.service` |
...
| 項目 | 値 |
|---|---|
| 本体スクリプト | `/usr/local/bin/vm-health-monitor.sh` |
| 設定ファイル | `/etc/vm-health-monitor/targets.conf` |
| systemd unit | `vm-health-monitor.service` |
| 周期 | 60 秒 |
