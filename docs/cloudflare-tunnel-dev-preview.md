...
- `X-Robots-Tag: noindex` を nginx ingress server-snippet で dev-* host だけに付与しやすい
- Django で `request.get_host()` が `dev-*` の時に `/robots.txt` を `Disallow: /` で返す分岐を素直に書ける
- LAN / 外部 で cookie domain や CSRF origin を分離しやすい

## 検索エンジン除外 (二重)

1. **nginx ingress server-snippet** (dev-* 専用 Ingress リソース側):
   ```
...
`k8s/base/ingress.yaml` に dev-* 3 host を含む別 Ingress リソース (noindex annotation 付き) を定義。Django settings の `ALLOWED_HOSTS` / `CSRF_TRUSTED_ORIGINS` に dev-* を追加。`/robots.txt` view で Host 分岐。
