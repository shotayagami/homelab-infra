...
> **現状ステータス**: 2026-05-18 時点で **稼働中** (192.168.11.174、Docker Compose スタック起動済)。Cloudflare Tunnel `home-yagamin` で `puter.yagamin.net` / `api.yagamin.net` / `site.yagamin.net` / `host.yagamin.net` / `app.yagamin.net` / `dev.yagamin.net` を公開。
...
- [docs/proxmox-zabbix-monitoring.md](proxmox-zabbix-monitoring.md) — Zabbix での Puter LXC 監視
